...
class UltimateSemrushClone {
        ...
        for (const route of staticRoutes) {
            try {
                ...
                if (textHTMLRatio < 0.1) {
                    this.addIssue('Content Quality', 'LOW', route.file,
                ...
                if (h1Count === 0) {
                    ...
                    this.addIssue('Structure', 'LOW', route.file,
                        `Multiple H1 tags (${h1Count}) in rendered page (likely responsive duplication)`);
                }
    ...
    private checkStructure(content: string, file: string) {
        ...
        if (h1Count === 0 && heroUsage === 0 && this.mode === 'static') {
            ...
            this.addIssue('Structure', 'LOW', file, `Multiple H1 tags (${h1Count})`);
        }
    }
    ...
    private checkContentQuality(content: string, file: string) {
        ...
        if (ratio < CONFIG.validation.contentRatioMin) {
            this.addIssue('Content Quality', 'LOW', file,
    ...
    private isValidInternalLink(href: string): boolean {
        ...
        if (this.validRoutes.has(cleanHref)) return true;

        const possiblePath = path.join(this.projectRoot, 'app', cleanHref.replace(/^\//, ''), 'page.tsx');
